...
# Agency introduces the 'Squiggle' into the structural scaffold [cite: 1227]
lattice_growth = fluid_growth * (k_geo + delta_agency)
sim_mass = old_world_limit + (lattice_growth - 1.0) * 2.5
print(f"Agency-Enhanced Match Rate: {np.sum(sim_mass >= observed_mass)/trials * 100:.2f}%")