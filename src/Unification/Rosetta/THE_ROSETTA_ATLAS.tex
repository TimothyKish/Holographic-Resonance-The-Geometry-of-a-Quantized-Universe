...
\definecolor{theorytag}{RGB}{150, 110, 0}
\definecolor{empiricaltag}{RGB}{0, 110, 60}
\definecolor{proventag}{RGB}{20, 90, 160}

% THREE-TIER EPISTEMIC TAGS (Vol 10/11 + Theorem update)
\newcommand{\TheoryTag}{\;\colorbox{theorytag!15}{\textcolor{theorytag}{\footnotesize\bfseries [\,THEORY / REACH\,]}}}
\newcommand{\EmpiricalTag}{\;\colorbox{empiricaltag!15}{\textcolor{empiricaltag}{\footnotesize\bfseries [\,EMPIRICAL\,]}}}
\newcommand{\ProvenTag}{\;\colorbox{proventag!15}{\textcolor{proventag}{\footnotesize\bfseries [\,PROVEN\,]}}}
...
\vspace{1em}
\noindent\textbf{\textcolor{kishblue}{A Note on the Three Tiers (Reading This Atlas).}}
This Atlas is a lean index to two houses of the same programme, and each entry is
tagged so the reader always knows which house it belongs to. \TheoryTag\ marks
\emph{reach}: motivated geometric constructions --- the Master Equation, the Kish
Clutch, the lattice mechanical quantities, the anthropic-time conversion, the
speed-of-light derivation --- that are not yet falsifiable in a direct empirical test.
\EmpiricalTag\ marks survey results that cleared the pipeline and the statistical
nulls, reproducible from open code and public data. \ProvenTag\ marks the single class
of result that is neither reach nor survey but a mathematical fact with a checkable
derivation. Both houses have a purpose. Empirical work without reach only repeats
itself; theory without an empirical test only stagnates. The tags do not rank the
entries --- they locate them. Reach earns its place by generating tests; empirical
earns its place by surviving them; and exactly one result, the Grid-Phase Locking
Theorem, is proven. A tagged entry points to its full treatment in the corresponding
volume or side paper; the Atlas itself stays deliberately lean and makes the quick point.
...
\section{The Baseline Lattice Equation (Without Agency Offset)\TheoryTag}
...
\section{The Enhanced Equation (With Life Agency Offset)\TheoryTag}
...
\section{The Register Family Atlas\EmpiricalTag}

Based on the survey record through Volume 11 --- 24{,}904{,}223 physical measurements across 52 active domains, 33 confirmed STRONG signals spanning 41 orders of magnitude (the Volume 9 baseline was 9.9 million records across 29 domains) --- the following table serves as the definitive map of the $N/\pi$ register space. 
...
\section{The Scalarization Rosetta: Operational Transforms\EmpiricalTag}
...
% ==============================================================================
% DROP-IN: THE LATTICE ENERGY EQUATION AND THE DERIVATION OF f
% Insert after the section "Resonance, Drag, and Slipstream" (the slipstream
% material), where f as the slipstream factor is discussed qualitatively. This
% section supplies the quantitative definition, ported from Volume 13 Chapter 5.
% Matches Atlas voice: declarative subsections, \TheoryTag / \ProvenTag,
% plain \[...\] display math.
% ==============================================================================
\section{The Lattice Energy Equation and the Derivation of $f$\ProvenTag}

Earlier forms of this Atlas carried the Lattice Energy of a mass $m$ at radius
$r$ as
\[
E_{\text{lat}} = \left(\frac{16}{\pi}\right)\!\left(\frac{m}{r^2}\right) f,
\]
with $f$ named --- but not defined --- as the ``slipstream factor.'' Volume~13
closed this gap by derivation rather than assertion. That derivation is ported
here as the canonical definition, and the ambiguity is retired.

\subsection{The Problem $f$ Posed}

A dimensional audit fixes the requirement: $E_{\text{lat}}$ is an energy only if
$f$ carries dimensions $[L^4][T^{-2}]$. But the word ``slipstream,'' used
everywhere else in this Atlas, denotes a \emph{dimensionless} drag factor bounded
on $[0,1]$. The two readings are incompatible, and a third --- $f$ as a stand-in
for the gravitational constant --- fails because the pure number $16/\pi$ cannot
carry the dimensions of $G$. As long as $f$ absorbed whatever the equation
required, the Master Equation's energy form was underdetermined.

\subsection{The One Postulate That Resolves It}

Dimensional analysis alone cannot fix the radial power: since $\ell_{\text{cell}}/r$
is dimensionless, $f = \ell_{\text{cell}}^2 c^2 (\ell_{\text{cell}}/r)^n$ satisfies
the dimensions for every $n$. A geometric principle is required, and the lattice
already supplies it.

\begin{quote}
\textbf{Postulate.} The lattice bulk is four-dimensional in space. This is stated
throughout the corpus --- the mapping of a 4D hypercube onto a cyclic time loop ---
it is embodied in the 24-cell, a four-dimensional regular polytope, and it is
already used in the mode count $N_{\text{total}} = d^2 = 4^2 = 16$ that produces
the modulus itself.
\end{quote}

\subsection{The Derivation, by Gauss's Law in Four Dimensions}

For a point source of strength $Q$ in $d$ spatial dimensions, flux conservation
through the enclosing $(d{-}1)$-sphere gives the field, and its integral gives the
potential:
\[
E(r) = \frac{Q}{\Omega_{d-1}\,r^{\,d-1}},
\qquad
V(r) = \frac{Q}{(d-2)\,\Omega_{d-1}}\cdot\frac{1}{r^{\,d-2}}
\quad (d \neq 2),
\]
where $\Omega_{d-1}$ is the surface measure of the unit $(d{-}1)$-sphere. For the
four-dimensional bulk, $d=4$ and $\Omega_3 = 2\pi^2$:
\[
V(r) = \frac{Q}{4\pi^2\,r^2}.
\]
\textbf{The potential of a point source in four spatial dimensions falls as
$1/r^2$} --- exactly the form of the Lattice Energy equation. The radial power is
$n=0$, derived rather than assumed. With the source strength set by the lattice
scale, $Q = \kappa\,m\,\ell_{\text{cell}}^2 c^2$, the interaction energy is
\[
E_{\text{lat}} = m\,V(r)
= \frac{\kappa}{4\pi^2}\;m c^2 \left(\frac{\ell_{\text{cell}}}{r}\right)^2,
\]
and matching the Atlas coefficient $16/\pi$ fixes the dimensionless coupling:
\[
\frac{\kappa}{4\pi^2} = \frac{16}{\pi}
\qquad\Longrightarrow\qquad
\boxed{\;\kappa = 64\pi = 4\,N_{\text{total}}\,\Phi_{\text{cycle}}\;}
\]
--- four times the metric-component count times the cyclic phase.

\subsection{The Canonical Form}

The Lattice Energy equation is therefore, with $f$ now defined:
\[
\boxed{\;
E_{\text{lat}}
= \frac{16}{\pi}\,m c^2 \left(\frac{\ell_{\text{cell}}}{r}\right)^2,
\qquad
f = \ell_{\text{cell}}^2\,c^2\;
\;}
\]
The name ``slipstream factor'' attaches properly to the separate,
phenomenological drag term $F(d,T)$ that modulates resonant motion --- not to
$f$, which is the geometric carrier derived above.

\subsection{The Cost, Stated Plainly\TheoryTag}

The derivation is honest only if its price is stated as clearly as its result.
We observe \emph{three} spatial dimensions, and both gravity and electrostatics
fall as $1/r$, not $1/r^2$. \textbf{A $1/r^2$ lattice energy therefore cannot be
the interaction measured at ordinary scales.} It can only be a bulk quantity
whose projection onto the three-dimensional boundary recovers $1/r$ --- and that
bulk-to-boundary projection is the framework's central open problem. The
holographic language of the corpus gestures at it; it does not yet perform it.

The framework has thus traded a \emph{vague} gap for a \emph{precise} one: from an
undefined $f$ and an unfixed radial power, to a derived $f = \ell_{\text{cell}}^2 c^2$,
a derived $n=0$, a derived coupling $\kappa = 64\pi$, and one well-posed open
question a competent theorist can attack.

\subsection{$\ell_{\text{cell}}$ Is a Postulate, Not a Result\TheoryTag}

Every input to the modulus is dimensionless: the mode count $N_{\text{total}}=16$,
the cyclic phase $\Phi_{\text{cycle}}=\pi$, the 24-cell packing. \textbf{No length
can be derived from dimensionless inputs.} The cell spacing $\ell_{\text{cell}}$
is therefore imported, not derived --- it is set to the Planck length by postulate,
which brings $\hbar$, $G$, and $c$ in from outside the framework. This is not a
defect peculiar to the lattice; every discrete-spacetime programme faces it. It
must be stated as a postulate, and it is now stated as one.

% ------------------------------------------------------------------------------
\section{Errata Against Earlier Atlas Forms\ProvenTag}

The derivation above supersedes three earlier statements in this Atlas. They are
recorded rather than silently overwritten, in keeping with the programme's
practice of publishing its corrections.

\subsection{Erratum RA--1: The Newtonian Analogy}

Earlier text described the Lattice Energy as ``mirroring Newton's $F = Gm/r^2$.''
This is withdrawn. $Gm/r^2$ is a field with the dimensions of acceleration,
compared against an energy; gravitational potential energy scales as $1/r$, not
$1/r^2$; and $G$ carries dimensions while $16/\pi$ is a pure number, making the
stated substitution dimensionally impossible. The Lattice Energy is a
four-dimensional point-source potential, not a gravitational analogue, and
$16/\pi$ does not replace $G$.

\subsection{Erratum RA--2: The Unit-Dependent Cell Spacing}

The chain $c = \ell_{\text{cell}} f_{24}$ with $f_{24}$ fixed to a laboratory
frequency implies a cell spacing of order $10^7$~m, in conflict with the Planck
identification by roughly forty-two orders of magnitude. That chain privileges the
second --- a caesium-hyperfine human convention --- and is therefore
unit-dependent. It is withdrawn. The relation $c = \ell_{\text{cell}} f_{24}$
remains a valid \emph{factorisation} of $c$ once $\ell_{\text{cell}}$ is fixed by
postulate, but it does not by itself \emph{derive} the value of $c$: one equation
in two unknowns fixes neither.

\subsection{Erratum RA--3: ``Only $16/\pi$ Produces Universal Stability''}

Earlier text asserted that offset-$\pi$ tests confirm ``only $16/\pi$ produces
universal stability.'' This overstates the empirical record and is corrected. The
degeneracy result of Volume~12 established that the register index and the modulus
enter the lock statistic only through their product, so a modulus scan and a
register scan trace the same one-parameter family --- the data can identify a
preferred product, not a preferred coordinate. And the first clean modulus sweep
against tens of thousands of alternatives, on amino-acid bond lengths in
Volume~13, found $16/\pi$ \emph{not} privileged: a randomly chosen modulus matched
or beat it in the majority of cases. \textbf{The specific value $16/\pi$ rests on
the geometric derivation --- the mode count $N_{\text{total}}/\Phi_{\text{cycle}}
= 16/\pi$ --- not on an empirical demonstration that it uniquely produces
stability.} That derivation is the claim; the empirical uniqueness assertion is
withdrawn.
...
\section{The Full 24-Harmonic Cutoff Derivation of the Speed of Light\TheoryTag}
...
\subsection{Lattice Energy Updated August 4th 2026 with new discoveries (Newtonian Form): The Core Translation Equation}
 
\begin{center}
{\color{proventag}\textbf{[EMPIRICAL core \textbullet\ dimensional footing PROVEN \textbullet\ electron-scale OPEN]}}
\end{center}
 
\subsubsection{Definition}
This is the bridge equation of the entire framework: the expression that translates
the lattice ontology into the Newtonian language of force and energy, so that a reader
who has never left classical mechanics can see what the $16/\pi$ lattice is doing in
terms they already own. In the lattice ontology, energy is not a scalar bottled inside
an object. It is the \emph{work of displacement} --- the effort required to push a mass
through a geometric tensegrity truss whose stiffness is the vacuum modulus $16/\pi$.
$E_{\mathrm{lat}}$ is not the energy \emph{of} a particle, nor a static background energy
\emph{stored} in a region. It is the interaction work spent against the grid's resistance
as a mass is moved through it.
 
\subsubsection{Expression}
\[
\boxed{\;E_{\mathrm{lat}} \;=\; \left(\frac{16}{\pi}\right)\left(\frac{m}{r^{2}}\right) f\;}
\]
 
\subsubsection{Breaking the Equation: Every Component in Newtonian Terms}
 
The equation is deliberately built to mirror Newton's law of gravitation,
$F = G\,\dfrac{m_1 m_2}{r^{2}}$, so that each lattice term has a classical shadow. We
break every component in turn.
 
\begin{itemize}
  \item \textbf{$\dfrac{16}{\pi} \approx 5.093$ --- the vacuum stiffness modulus.}
  This is the lattice's analogue of Newton's gravitational constant $G$. Where $G$ sets
  the strength of attraction, $16/\pi$ sets the \emph{stiffness} of the vacuum --- its
  resistance to being displaced. It is a pure, \textbf{dimensionless} geometric ratio:
  the number $16$ (the closure count of the $24$-cell's structural sub-geometry) divided
  by $\pi$ (the curvature of the resonance). It carries no units. This is essential:
  because it is dimensionless, all the physical dimensions of $E_{\mathrm{lat}}$ must
  come from $m$, $r$, and $f$.
 
  \item \textbf{$\dfrac{m}{r^{2}}$ --- the Newtonian displacement kernel.}
  This is lifted directly from the inverse-square law. $m$ is the interacting mass
  $[\mathrm{M}]$; $r$ is the geometric separation, or lever-arm radius, through which the
  displacement is measured $[\mathrm{L}]$. The ratio $m/r^{2}$ has dimensions
  $[\mathrm{M}][\mathrm{L}^{-2}]$ --- the same kernel that appears in a gravitational
  field. In Newton it produces attraction; here it produces the \emph{geometric grip}
  of the truss on the mass.
 
  \item \textbf{$f$ --- the slipstream factor (drag-corrected motion through the lattice).}
  This is the term the classical picture has no name for, and it is where the lattice
  ontology adds something Newton could not. $f$ describes how the mass's passage is
  modified by drag as it moves through the truss --- the ``slipstream'' of a body
  carving a wake through the geometric medium, the same image Volume~5 used for a
  satellite in the thin upper atmosphere or a swimmer in a wake. Crucially, $f$ is
  \textbf{not} a dimensionless drag \emph{coefficient}. For $E_{\mathrm{lat}}$ to be a
  literal energy, $f$ must carry dimensions --- and that requirement pins what $f$
  physically is (next section).
\end{itemize}
 
\subsubsection{Dimensional Closure: Fixing $f$ So the Equation Is Literally Energy}
 
The original Atlas entry left $f$'s dimensions ambiguous, calling it a ``factor.'' A
dimensionless factor would make $E_{\mathrm{lat}}$ come out as
$[\mathrm{M}][\mathrm{L}^{-2}]$ --- \emph{not} an energy. We resolve the ambiguity here.
Energy has dimensions $[\mathrm{M}][\mathrm{L}^{2}][\mathrm{T}^{-2}]$. Solving for $f$:
 
\[
[\,f\,] \;=\; \frac{[\mathrm{M}][\mathrm{L}^{2}][\mathrm{T}^{-2}]}
{\big(\tfrac{16}{\pi}\big)\,[\mathrm{M}][\mathrm{L}^{-2}]}
\;=\; [\mathrm{L}^{4}][\mathrm{T}^{-2}].
\]
...
\textbf{Term} & \textbf{Dimensions} & \textbf{Role} \\
\hline
$16/\pi$ & dimensionless & vacuum stiffness (analogue of $G$) \\
$m$ & $[\mathrm{M}]$ & interacting mass \\
$r^{2}$ & $[\mathrm{L}^{2}]$ & geometric separation squared \\
$m/r^{2}$ & $[\mathrm{M}][\mathrm{L}^{-2}]$ & Newtonian displacement kernel \\
$f$ & $\mathbf{[\mathrm{L}^{4}][\mathrm{T}^{-2}]}$ & \textbf{kinematic slipstream flux} \\
\hline
\rowcolor{empiricaltag!15}
$E_{\mathrm{lat}}$ & $[\mathrm{M}][\mathrm{L}^{2}][\mathrm{T}^{-2}]$ & \textbf{energy (closes)} \\
\hline
\end{tabular}
\end{center}
 
With $f = [\mathrm{L}^{4}][\mathrm{T}^{-2}]$ and $16/\pi$ dimensionless, the equation
closes exactly to energy. This is the corrected footing. It also \emph{reclassifies}
$f$: the dimensions factor as
\[
[\mathrm{L}^{4}][\mathrm{T}^{-2}] \;=\; \big([\mathrm{L}^{2}][\mathrm{T}^{-1}]\big)^{2}
\;=\; (\text{area flux rate})^{2}
\;=\; (\text{area}) \times (\text{speed})^{2}.
\]
The cleanest physical reading is $f$ as a \textbf{kinematic flux-area rate, squared}:
the square of the rate at which the moving mass sweeps lattice area. This is a genuinely
\emph{kinematic} quantity --- a property of motion through the grid --- not a static
ratio. The ``slipstream'' name is kept; its \emph{type} is fixed as kinematic.
 
\subsubsection{Empirical Footing: The Kinematic Band Across Volumes 5--11}
 
The reclassification of $f$ as kinematic is not a dimensional convenience --- it is what
the survey confirms. Motion through the lattice locks at the \textbf{kinematic register},
and it does so as one of the most reproduced results in the entire framework.
...
\hline
\rowcolor{kishblue!12}
\textbf{Volume} & \textbf{Motion quantity} & \textbf{Register} & \textbf{Peak $z$} \\
\hline
Vol 5  & Stellar transverse velocity  & $16/\pi$ & $+94$ \\
Vol 5  & Tidal intervals (NOAA)       & $16/\pi$ & $+21$ \\
Vol 7  & Stellar transverse velocity  & $16/\pi$ & $+102.8$ \\
Vol 8  & Stellar velocity / solar cycle & $16/\pi$ & $+106.7$ \\
Vol 9  & Stellar kinematic            & $16/\pi$ & $+100.8$ \\
Vol 10 & Stellar kinematic            & $16/\pi$ & $+99.6$ \\
Vol 10 & Orbital TTV (Kepler)         & $16/\pi$ & $+78.5$ \\
Vol 11 & Stellar kinematic (multi-attr) & $\mathbf{15/\pi}$ & $+98.3$ \\
Vol 6  & Galactic velocity dispersion & $\mathbf{15/\pi}$ & $+34.5$ \\
\hline
\end{tabular}
\end{center}
 
\paragraph{The corrected empirical edge --- the constant is a band, not a point.}
Six independent volumes place stellar transverse velocity --- pure motion through the
lattice, exactly the quantity $E_{\mathrm{lat}}$ describes --- at $16/\pi$ with
$z \approx 100$. But the register is \emph{not} a razor-sharp point. In the Volume~11
multi-attribute run, the same stellar velocity landed at the \emph{adjacent} member
$15/\pi$ ($z = 98.3$), and galactic velocity dispersion sits at $15/\pi$ as well. The
framework's own taxonomy already names this: the \textbf{kinematic family}, a band whose
core is the pair $15/\pi$--$16/\pi$ ($4.775$--$5.093$), primary at $16/\pi$. Which member
a given motion occupies depends on the measurement's selection criteria (resolution,
attribute), as Volume~11's resolution-sensitivity discovery made explicit.
 
\begin{center}
\begin{tikzpicture}[scale=1.0]
% number line of registers
\draw[->, thick] (0,0) -- (12,0) node[right] {\small $N/\pi$ register};
\foreach \x/\lab in {1/{14},3/{15},5.5/{16},8/{17},10/{18}} {
  \draw[kishblue, thick] (\x,-0.12) -- (\x,0.12);
  \node[below] at (\x,-0.18) {\scriptsize $\lab/\pi$};
}
% the kinematic band highlighted (15-16)
\fill[goldseal!25] (2.6,-0.9) rectangle (5.9,1.9);
\node[goldseal!80!black, font=\small\bfseries] at (4.25,2.2) {KINEMATIC BAND};
% primary marker at 16
\draw[->, very thick, kishblue] (5.5,1.6) -- (5.5,0.2);
\node[kishblue, font=\scriptsize, above] at (5.5,1.6) {PRIMARY};
\node[kishblue, font=\scriptsize] at (5.5,0.9) {stellar $v_\perp$};
\node[kishblue, font=\scriptsize] at (5.5,0.55) {$z\!\approx\!100$};
% adjacent marker at 15
\draw[->, very thick, luminorange] (3.0,1.3) -- (3.0,0.2);
\node[luminorange, font=\scriptsize, above] at (3.0,1.3) {ADJACENT};
\node[luminorange, font=\scriptsize] at (3.0,0.75) {galactic disp.};
\node[luminorange, font=\scriptsize] at (3.0,0.4) {$z\!=\!34.5$};
\node[align=center, font=\scriptsize, gray] at (9.0,1.3)
  {\;selection criteria\\ \;shift the member\\ \;within the band};
\end{tikzpicture}
\end{center}
 
\subsubsection{Interpretation: Answering the Two Questions}
 
Two questions are frequently asked of this equation, and the tightened definition answers
both directly.
 
\begin{itemize}
  \item \textbf{Does $E_{\mathrm{lat}}$ belong to a propagating particle, or is it the
  background energy stored in a region?} Neither. It is \emph{interaction work} --- the
  work done against lattice stiffness as a mass is displaced through the truss. It is a
  process quantity (work of displacement), not a carrier quantity (particle energy) and
  not a state quantity (stored background). This follows from the definition and is
  reinforced by the kernel $m/r^{2}$: the term is intrinsically about the grip on a mass
  at a separation, i.e.\ about displacement, not storage.
 
  \item \textbf{Is $f$ a fundamental constant of the grid, or is it generated by the mass
  vibrating?} Neither. The fundamental grid constant in this equation is $16/\pi$ --- the
  stiffness modulus, fixed and dimensionless. $f$ is \emph{relational}: a kinematic
  slipstream flux $[\mathrm{L}^{4}][\mathrm{T}^{-2}]$ describing how the mass's motion is
  drag-corrected as it moves through the grid. It is a property of the
  mass-grid \emph{interaction}, not of the grid alone and not generated by the mass alone.
\end{itemize}
 
\subsubsection{Scope and Honest Limits}
 
\begin{center}
{\color{empiricaltag}\textbf{[EMPIRICAL --- macroscopic]}}\quad
{\color{theorytag}\textbf{[OPEN --- electron scale]}}
\end{center}
 
The empirical footing of $E_{\mathrm{lat}}$ is strong \emph{and bounded}, and honesty
requires stating both:
 
\begin{itemize}
  \item \textbf{What is confirmed.} Macroscopic mass moving through the lattice --- stars,
  galaxies, tides, planetary orbits --- locks in the kinematic band ($15/\pi$--$16/\pi$),
  reproduced across six volumes at $z \approx 100$. The dimensional closure is exact. The
  drag/slipstream reading is the volumes' own language for these results. On this scale,
  $E_{\mathrm{lat}}$ stands on the firmest empirical ground in the framework.
 
  \item \textbf{What is corrected.} The constant $16/\pi$ is the \emph{primary} of a
  kinematic \emph{band}, not a pinned point value. The adjacent member $15/\pi$ is
  confirmed under different selection criteria. $E_{\mathrm{lat}}$ uses $16/\pi$ as the
  canonical kinematic stiffness; the band-width is real and is stated, not hidden.
 
  \item \textbf{What is open.} Every confirmation above is \emph{macroscopic} motion.
  Whether $E_{\mathrm{lat}}$'s kinematic grounding extends to \emph{electron-scale}
  kinematics is an open question --- and the current Sister-Papers probe fleet finds that
  the kinematic electron (beta-decay and conduction probes) does \emph{not} lock at
  $16/\pi$. This does not weaken the macroscopic footing, but it forbids extending
  $E_{\mathrm{lat}}$ to the electron without further evidence. The $f = (\text{area
  flux})^{2}$ reading is offered as the interpretation most consistent with the confirmed
  macroscopic kinematics; it is not claimed as dimensionally unique.
\end{itemize}
 
\subsubsection{Usage}
Used to translate lattice behaviour into classical mechanics for comparison, simulation,
or pedagogy --- and, with $f$ now dimensionally fixed, as a literal energy expression for
macroscopic mass displaced through the lattice. It is the Rosetta bridge between classical
gravity (force as attraction, $F = Gm/r^{2}$) and lattice mechanics (energy as work of
displacement through a tensegrity grid of stiffness $16/\pi$). Cite with its scope: proven
footing at macroscopic scale, band-primary at $16/\pi$, electron scale open.
...
\section{Lattice Pressure\TheoryTag}
...
\section{Lattice Impedance\TheoryTag}
...
\section{Temporal Packet\TheoryTag}
...
\chapter{The 2D Zeta-Prime Clock: Forming, Not Yet Formed\TheoryTag}

Through Volume 11, the horizontal axis of the lattice coordinate system --- the $\sigma$ scalar axis defined by the $N/\pi$ harmonic registers --- is empirically established: physical domains cluster at these discrete geometric nodes across 52 domains and 41 orders of magnitude, and the Grid-Phase Locking Theorem proves those clusters are genuine phase-concentration rather than transform artifacts. The vertical axis is a different matter, and this chapter is deliberately honest about its status: \emph{it is forming out of the data, but it is not yet formed}.
...
\vspace{0.5em}
\noindent\textbf{\textcolor{theorytag}{Current status --- forming, not formed.}}
The $\tau$ axis is mostly defined empirically with one piece missing: the surveyed
registers show the beginnings of a coupling between a domain's structural register and a
prime-indexed cadence, but not all registers required to fix the coupling function are yet
in hand. Mondy's assessment is that the structure will most likely form on its own as the
remaining registers are surveyed --- but this is a suspicion held under discipline, not a
result, and it is explicitly \emph{not being forced}. The 2D clock is theorized to exist
and to bear a relationship to the fifth and the resonant zeta primes; that relationship is
a \TheoryTag\ construction awaiting the engine built to expose it. If the remaining
registers arrive and the coupling does not form, that null is as publishable as a
confirmation. We follow the data; we do not pull it into shape. The dual-register
co-dominance already observed in the survey --- subnuclear $22/\pi$ \& $13/\pi$, protein
backbone $16/\pi$ \& $25/\pi$, the Gaia $12/15/16$ band --- is the candidate empirical
shadow of this second dimension projected onto the one-dimensional register line, and is
the concrete, pre-registerable hypothesis the new 2D engine is being built to test.
...
The 1D register axis is established and, for Form~A, proven. Whether the 2D clock locks is an open empirical question the forthcoming 2D engine is designed to settle --- in either direction. If it forms, it unifies the spatial geometry of the lattice with its temporal cadence; if it does not, that too enters the sovereign record as returned.
...
physical register assignments confirmed across 24.9~million measurements (Volume 11 record; 9.9~million at the Volume 9 baseline).

\section{The Kish Clutch: The Scale-Bridging Operator\TheoryTag}
...
\section{The Chaos Null and the Spectroscopic Definition\EmpiricalTag}
...
\subsection{The Grid-Phase Locking Theorem\ProvenTag}

The chaos-null lock test above is a \emph{proximity-to-grid} test: a scalar locks when
it falls within tolerance $\tau=0.05$ of a node of the register grid. This one structural
fact yields a theorem --- the only \ProvenTag\ entry in this Atlas. Full treatment:
\emph{Noise Does Not Lock} (v2.0) and KLGHS Volume 11 (v4.0).

\vspace{0.4em}
\noindent\textbf{Offset-Average Identity.} For a grid of spacing $\Delta$ and any
distribution $D$, let $L(D_\theta)$ be the lock fraction after shifting $D$ by offset
$\theta$. Then the offset-average over one grid period is fixed, for every distribution:
\begin{equation}
\frac{1}{\Delta}\int_{0}^{\Delta} L(D_\theta)\,d\theta \;=\; 2\tau \;=\; 0.1 .
\label{eq:offset_identity}
\end{equation}
(One-line proof: exchange the average over data with the average over offset; each fixed
point sweeps uniformly through one cell and contributes exactly the locked-band measure
$2\tau$.) Verified numerically at $0.1000$.

\vspace{0.4em}
\noindent\textbf{Theorem.} No distribution can lock strongly ($L>2\tau$) at \emph{every}
offset. If it did, its offset-average would exceed $2\tau$, contradicting
Eq.~\eqref{eq:offset_identity}. Therefore a \emph{shape-artifact} --- a strong lock
produced by distribution shape alone, independent of grid phase --- cannot exist. The
transform-shape objection (Form~A) is not merely unsupported; it is vacuous.

\vspace{0.4em}
\noindent\textbf{Scope --- load-bearing, not a footnote.} The theorem closes Form~A and
only Form~A: it proves a lock is genuine grid-phase concentration, not an artifact of the
transform. It does \emph{not} prove the lock is caused by the lattice. That question ---
Form~B, scale-coincidence and selection --- remains the open primary objection and is
named in the same breath every time this theorem is cited. The identity also predicts the
signature confirmed on three real anchors (Gaia $16/\pi$, galactic $21/\pi$, nuclear
$21/\pi$): a sharp peak at the privileged offset is compensated by \emph{deeply negative}
median $z$ across other offsets, because the offset-average is pinned at $2\tau$. Disproof
condition, open to any critic: exhibit a distribution that locks at $z\ge 5$ and stays
locked across a full offset sweep.

\section{Domain Rosetta Blocks\EmpiricalTag}
...
\section{The Rosetta at Scale: Summary Table\EmpiricalTag}
...
\vspace{1em}
\noindent\textbf{Volume 10: \emph{The Deep Survey}}\\
\href{https://doi.org/10.5281/zenodo.20587181}{See the KLGHS Volume 10 record on Zenodo}

\vspace{1em}
\noindent\textbf{Volume 11: \emph{The Structure That Locks}} (versions 1.0--4.0; concept DOI resolves to latest)\\
\emph{v1.0} \href{https://doi.org/10.5281/zenodo.20587181}{10.5281/zenodo.20587181} \quad
\emph{v2.0} \href{https://doi.org/10.5281/zenodo.21199809}{10.5281/zenodo.21199809}\\
\emph{v3.0} \href{https://doi.org/10.5281/zenodo.21210675}{10.5281/zenodo.21210675} \quad
\emph{v4.0 (Grid-Phase Locking Theorem)} \href{https://doi.org/10.5281/zenodo.21212886}{10.5281/zenodo.21212886}

\vspace{1em}
\noindent\textbf{The Geometric Electron: The Reclassification of the Electron as a Lattice Vertex}\\
\emph{v3.0} \href{https://doi.org/10.5281/zenodo.21611448}{10.5281/zenodo.21611448}

\vspace{1em}
\noindent\textbf{The Orthogonal Torque: Redefining Magnetism as Lattice Torsion}\\
\emph{v4.0} \href{https://doi.org/10.5281/zenodo.21611531}{10.5281/zenodo.21611531}

\vspace{1em}
\noindent\textbf{Companion (Critics' Paper): \emph{Noise Does Not Lock}} (v1.0; v2.0 adds the theorem chapter)\\
\emph{Concept DOI (all versions)} \href{https://doi.org/10.5281/zenodo.20587180}{10.5281/zenodo.20587180}
...
{\large The project is now live on the web. Researchers, skeptics, and replicators are invited in:}\\[0.4em]
{\large\href{https://www.kishlattice.com}{www.kishlattice.com}}

\vspace{0.5em}

\noindent The code is open, the data is public, and the falsification conditions are explicit. If you can exhibit a distribution that locks at $z\ge 5$ and stays locked across a full offset sweep, the Grid-Phase Locking Theorem falls. The invitation to try is genuine.\
