...
def validate():
    print("=" * 60)
    print("VALIDATE: p2_orbital_radius")
    print("=" * 60)
    print()
    ...
    if n_total < 5_000:
        warnings.append(
            f"Only {n_total:,} records — expected ~5,000-13,500. "
            "Not all planets have measured semi-major axis — this is expected."
        )
    ...
    if scalars:
        ...
        if ratio > 0.28:
            ...
            warnings.append(
                ...
                "Orbital radius spans many orders of magnitude (hot Jupiters "
                "to wide-orbit giants), producing natural spread."
            )
        else:
            print("    PASS ✓")
    print()
    ...
    print("  ─────────────────────────────────────────────────────")
    print('  "p2_orbital_radius": {')
    print('    "path": "lakes/inputs_promoted/p2_orbital_radius_promoted.jsonl",')
    print('    "enabled": true,')
    print('    "domain": "orbital",')
    print('    "scale_rank": 2,')
    print('    "__scalar__": "log(1 + pl_orbsmax_AU) / log(k_geo) — semi-major axis",')
    print('    "__source__": "NASA Exoplanet Archive pscomppars, same planets as P1",')
    print('    "__same_object__": ["p1_orbital_periods"],')
