...
\begin{itemize}
    ...
    \item Aschwanden, M. J. (2005). \textit{Physics of the Solar Corona: An Introduction}. Springer.
    ...
    \item Cowling, T. G. (1953). “Solar Electrodynamics.” In \textit{The Sun}, ed. G. P. Kuiper. University of Chicago Press.

    \item Gough, D. O. (1977). “Mixing-Length Theory for Stellar Convection.” \textit{Astrophysical Journal}, 214, 196–213.

    \item Holman, G. D. (1985). “Acceleration of Electrons in Solar Flares.” \textit{Astrophysical Journal}, 293, 584–594.

    \item Parker, E. N. (1955). “Hydromagnetic Dynamo Models.” \textit{Astrophysical Journal}, 122, 293–314.

    \item Parker, E. N. (1979). \textit{Cosmical Magnetic Fields: Their Origin and Activity}. Oxford University Press.

    \item Priest, E., \& Forbes, T. (2000). \textit{Magnetic Reconnection: MHD Theory and Applications}. Cambridge University Press.

    \item Schrijver, C. J., \& Zwaan, C. (2000). \textit{Solar and Stellar Magnetic Activity}. Cambridge University Press.

    \item Stix, M. (2002). \textit{The Sun: An Introduction}. Springer.

    \item Tayler, R. J. (1973). “The Adiabatic Stability of Stars Containing Magnetic Fields.” \textit{Monthly Notices of the Royal Astronomical Society}, 161, 365–380.
...
\begin{center}
\includegraphics[width=0.85\linewidth]{M3_Full_Lake_Audit.png}
\end{center}
...
\subsection*{Generated Image}
\begin{center}
\includegraphics[width=0.80\linewidth]{M4_Rosetta_Mapping_Chart.png}
\end{center}
...
\subsection*{Generated Image}
\begin{center}
\includegraphics[width=0.80\linewidth]{M5_Thermodynamics_Audit.png}
\end{center}
...
\subsection*{Generated Image}
\begin{center}
\includegraphics[width=0.80\linewidth]{M6_Stability_Differential_Chart.png}
\end{center}
...
\subsection*{Generated Image}
\begin{center}
\includegraphics[width=0.90\linewidth]{M7_Macro_Banding_Ladder.png}
\end{center}
...
\subsection*{Generated Image}
\begin{center}
\includegraphics[width=0.90\linewidth]{M9_Topological_Vortex.png}
\end{center}
...
(2) the exact sovereign script used,  
(3) terminal output,  
(4) the generated figure, and  
(5) a chain-of-custody note.  
...
\textbf{Purpose.}  
UMC1 extracts the raw Kish scalar values from the sovereign Materials and Chemistry lakes.  
No normalization or smoothing is applied. This establishes the empirical baseline:  
Chemistry is sharply peaked near zero, while Materials spans a broader range.

\section*{Source Code: umc1_scalar_compare.py}
...
\begin{lstlisting}[style=terminal]
python umc1_distributions.py
# (insert actual stdout)
\end{lstlisting}
...
All values sourced directly from sovereign JSONL lakes. No preprocessing or filtering.
...
\textbf{Purpose.}  
UMC2 rescales both scalar lakes to a shared normalized axis.  
This reveals the first region of cross-domain overlap (typically 0–1),  
where harmonic alignment begins to emerge.

\section*{Source Code: umc2_distributions.py}
\begin{lstlisting}[language=Python, caption={umc2_distributions.py}]
#!/usr/bin/env python3
...
\textbf{Chain-of-Custody.}  
Normalization is monotone; no information discarded.
...
\textbf{Purpose.}  
UMC3 computes density gradients to detect shelf boundaries—harmonic plateaus  
where the distribution flattens. Both domains exhibit shelf structure,  
confirming shared harmonic architecture.

\section*{Source Code: umc3_shelves.py}
\begin{lstlisting}[language=Python, caption={umc3\_shelves.py}]
#!/usr/bin/env python3
# UMC3 — Harmonic Shelves
# © 2026 KishLattice 16pi Initiative — Sovereign Audit Script
# Purpose: Detect harmonic shelf boundaries via density gradients.
"""
UMC3 — Harmonic Shelves
Materials ↔ Chemistry
...
\centering
\includegraphics[width=0.85\linewidth]{figures/umc/umc3_shelves.png}
\caption{U3.1 — Harmonic shelf detection across Materials and Chemistry.}
\end{figure}

\textbf{Chain-of-Custody.}  
Shelf detection uses histogram-based gradients only.
...
\textbf{Purpose.}  
UMC4 computes the second-order gradient of the normalized scalar distributions,  
producing the resonance map. This reveals the first nonlinear harmonic signature:  
a stable plateau, a resonance trough, and a rebound peak.

\section*{Source Code: umc4_resonance.py}
\begin{lstlisting}[language=Python, caption={umc4\_resonance.py}]
#!/usr/bin/env python3
# UMC4 — Resonance Map
# © 2026 The KishLattice 16pi Initiative — Sovereign Audit Script
# Purpose: Compute second-order resonance gradients.
"""
UMC4 — Resonance Map
Materials ↔ Chemistry
...
\centering
\includegraphics[width=0.85\linewidth]{figures/umc/umc4_resonance.png}
\caption{U4.1 — Cross-domain resonance map.}
\end{figure}

\textbf{Chain-of-Custody.}  
Resonance computed directly from normalized distributions.

% ------------------------------------------------------------------------------
% U5 — UMC5: COUPLING STRENGTH
% ------------------------------------------------------------------------------

\clearpage
\subsection*{U5 — UMC5: Coupling Strength}
\addcontentsline{toc}{subsection}{U5 — UMC5: Coupling Strength}

\textbf{Purpose.}  
UMC5 measures linear coupling between the resonance curves after symmetric resampling  
(N = 5000). The expected result is near zero, confirming that the domains are not trivially  
linearly related.

\section*{Source Code: umc5_coupling.py}
\begin{lstlisting}[language=Python, caption={umc5\_coupling.py}]
#!/usr/bin/env python3
# UMC5 — Coupling Strength
# © 2026 KishLattice 16pi Initiative — Sovereign Audit Script
# Purpose: Compute linear coupling coefficient after symmetric resampling.
"""
UMC5 — Coupling Strength
Materials ↔ Chemistry
...
\subsubsection*{Terminal Output}
\begin{lstlisting}[style=terminal]
python umc5_coupling.py
Coupling coefficient: -0.00775924899754941
\end{lstlisting)
...
\textbf{Chain-of-Custody.}  
Resampling uses monotone interpolation; no domain is privileged.
...
\textbf{Purpose.}  
UMC6 maps the shelf structures of the two domains into a nonlinear cascade.  
The characteristic spike-and-tail pattern demonstrates asymmetric but real harmonic coupling.

\section*{Source Code: umc6_cascade.py}
...
\textbf{Chain-of-Custody.}  
Shelf vectors resampled to shared grid (N = 5000).
...
\textbf{Purpose.}  
UMC7 collapses the entire ladder—distributions, shelves, resonance, coupling, and cascade—  
into a single scalar invariant. This value, \textbf{KLC–MC}, is the unified coordinate of the  
joint Materials–Chemistry lake on the Kish Lattice.
...
\textbf{Chain-of-Custody.}  
Computed directly from UMC1–UMC6 outputs.  
No external data, no smoothing, no transformations beyond the documented ladder.
